...
public class AppIntro {
	...
	public AppIntro(Activity ctx, int language) {
		...
		m_bitmapButtonWeb = BitmapFactory.decodeResource(ctx.getResources(),
				R.drawable.button_web);
	}
	...
	private boolean isConnectedToInternet() {
		ConnectivityManager cm = (ConnectivityManager) m_ctx
	...
	// t in [0..1]
	private void getCardioid(float t, float cx, float cy, float radiusBase,
			float radiusApple, V2d pointOut) {
		float phi;

		phi = 3.1415925636f * 2.0f * t;
		pointOut.x = (int) (cx + radiusBase * Math.sin(phi) + radiusApple
				* (Math.sin(phi) - Math.sin(2.0f * phi)));
		pointOut.y = (int) (cy - radiusBase * Math.cos(phi) - radiusApple
				* (Math.cos(phi) - Math.cos(2.0f * phi)));
	}
	...
	private void drawCircleInc(Canvas canvas, int deltaTimeMs) {
		...
		for (int a = 0; a < NUM_SEG_APPLE; a++) {
			getCardioid(t, m_scrCenterX, m_scrCenterY, radiusBase, radiusApple,
					m_point);
			t += tStep;
			if (a == 0)
	...
	private void drawAppleFillShader(Canvas canvas, int deltaTimeMs) {
		...
		RadialGradient gradientRadial = new RadialGradient(xSpot, ySpot,
				radGrad, colors, null, Shader.TileMode.CLAMP);
	...
	private void drawAppleLeaf(Canvas canvas, int deltaTimeMs) {
		...
		float yLeafSource = m_scrCenterY - m_appleRadiusBase
				- m_appleRadiusBase * 0.90f;
		...
		if (m_timeState > TIME_LEAF) {
			...
			m_paintTextWhite.getTextBounds(m_strDepth, 0, m_strDepth.length(),
					r);
			float h = r.height();
			float vOff = 0.0f;
			if (m_scrH > m_scrW)
				vOff = 3 * h;
			canvas.drawText(m_strDepth, 0, m_strDepth.length(), m_scrCenterX,
					vOff + h, m_paintTextWhite);
			canvas.drawText(m_strUniversity, 0, m_strUniversity.length(),
					m_scrCenterX, vOff + h * 2.5f, m_paintTextYell);
		}
		if ((m_timeState > 2 * TIME_LEAF) && isConnectedToInternet()) {
			...
			int bh = bw >> 2;
			if (m_scrH > m_scrW)
				rDst.set(m_scrCenterX - (bw >> 1), m_scrH - bh * 2,
						m_scrCenterX + (bw >> 1), m_scrH - bh);
			else
				rDst.set(m_scrCenterX - (bw >> 1), m_scrH - bh, m_scrCenterX
						+ (bw >> 1), m_scrH);
			canvas.drawBitmap(m_bitmapButtonWeb, rSrc, rDst, m_paintBitmap);
			m_paintTextWhite.setTextSize(20.0f);
			m_paintTextWhite
					.getTextBounds(m_strWeb, 0, m_strWeb.length(), rSrc);
			h = rSrc.height();
			canvas.drawText(m_strWeb, 0, m_strWeb.length(), rDst.centerX(),
					rDst.centerY() + (h >> 1), m_paintTextWhite);
		}
	...
	public boolean onTouch(int x, int y, int touchType) {
		...
		Rect rDst = new Rect();
		if (m_scrH > m_scrW)
			rDst.set(m_scrCenterX - (bw >> 1), m_scrH - bh * 2, m_scrCenterX
					+ (bw >> 1), m_scrH - bh);
		else
			rDst.set(m_scrCenterX - (bw >> 1), m_scrH - bh, m_scrCenterX
					+ (bw >> 1), m_scrH);
		if (rDst.contains(x, y)) {
			if (isConnectedToInternet()) {
				...
				Intent browserIntent = new Intent(Intent.ACTION_VIEW,
						Uri.parse("http://amd.stu.neva.ru/"));
